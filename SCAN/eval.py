...
from utils.common_config import get_val_dataset, get_val_transformations, get_val_dataloader,\
                                get_model, get_train_dataset, get_test_dataset
...
def main():
    ...
    #dataset = get_val_dataset(config, transforms)  for cifar10/cifar100
    #dataset = get_test_dataset(config, transforms) for cinic10
    ...
    if config['setup'] in ['simclr', 'moco']:
        print('Perform evaluation of the pretext task (setup={}).'.format(config['setup']))
        ...
        print('Perform evaluation of the clustering model (setup={}).'.format(config['setup']))
        ...
        #np.save(os.path.join(p["selflabel_dir"], "test_cluster_pred.npy"), predictions[0]['predictions'].detach().cpu().numpy())

        np.save(os.path.join(p["selflabel_dir"], "train_cluster_pred.npy"), predictions[0]['predictions'].detach().cpu().numpy())
