...
def x_u_split(args):
    ...
    total_indexs = np.load(os.path.join(MyPath.ckpt_root_dir(), "PATEFM", args.dataset, str(args.num_teachers)+'teachers', "partition.npy"))

    all_indices = np.arange(args.len_dataset)
    labeled_idx = total_indexs[args.teacher_id*args.num_labeled:(args.teacher_id+1)*args.num_labeled]
