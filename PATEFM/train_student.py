...
sys.path.append("./../")
from dataset.cifar_student import DATASET_GETTERS
...
def main():
    ...
    args.out = os.path.join(MyPath.ckpt_root_dir(), "PATEFM", args.dataset, str(args.num_teachers)+'teachers', 'eps'+str(args.epsilon), 'student_model')
...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.local_rank in [-1, 0]:
            ...
            logger.info("Train Epoch: {epoch}/{epochs:4}. LR: {lr:.4f}. Loss: {loss:.4f}. Loss_x: {loss_x:.4f}. Loss_u: {loss_u:.4f}. Mask: {mask:.2f}. ".format(
                    epoch=epoch + 1,
                    epochs=args.epochs,
                    lr=scheduler.get_last_lr()[0],
                    loss=losses.avg,
